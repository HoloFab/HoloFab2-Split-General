...
namespace HoloFab {
	public class ClientUpdater : UDPSend {
		...
		public bool ContainsID(int _id) {
			try { 
				return this.holoState.holoComponents.Any(component => component.id == _id);
			} catch {
		...
		public HoloComponent this[int _id]{
			get {
				try {
                    return this.holoState.holoComponents.First(component => component.id == _id);
                } catch {
					this.holoState = new HoloSystemState();
					return null;
				}
		...
		public ClientUpdater(object _owner, string _IP, int _port=8802) :
														base(_owner, _IP: _IP, _port: _port) {
            this.holoState = new HoloSystemState();
		}
		////////////////////////////////////////////////////////////////////////
